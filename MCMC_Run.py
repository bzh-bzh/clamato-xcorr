...
def one_dimensional_log_lik(self, params=None, direct_pk=None, return_aux=False):
    ...
    for name in self.corr_items:
        ...
        # Compute the normalizations here, since we've changed the dimension of the data.
        log_norm -= 0.5 * data_masked.size * np.log(2 * np.pi)
        if self.monte_carlo:
            raise NotImplementedError
        else:
            _, d, _ = scipy.linalg.ldl(reduced_cov)
            log_cov_det = np.log(d.diagonal()).sum()
            log_norm -= 0.5 * log_cov_det
    ...
    if return_aux:
        return log_lik, (raw_chi2, reduced_cov, mask_nonflat)
    else:
        return log_lik
