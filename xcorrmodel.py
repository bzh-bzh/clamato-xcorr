...
class ModelFunc:
    def __init__(self, ModArr):
        ...
        self.bias_lim = (np.min(self.bias), np.max(self.bias))
        self.sigz_lim = (np.min(self.sigz), np.max(self.sigz))
        ...
        # Generate interpolation functions per-bin for use in XCorrInterpRaw
        self.interpfns = []
        for ibin in np.arange(self.nbin):
            xi_arr_tmp = np.squeeze(self.xi_conv[:,:,ibin])
            self.interpfns.append(interpolate.RegularGridInterpolator((self.bias, self.sigz), xi_arr_tmp, method='linear',
    ...
    def XCorrInterpRaw(self, bias_in, sigz_in):
        ...
        """
        if bias_in < self.bias_lim[0] or bias_in > self.bias_lim[1]:
            print("Warning: input bias value outside parameter space. We are extrapolating...")

        if sigz_in < self.sigz_lim[0] or sigz_in > self.sigz_lim[1]:
        ...
        for ibin in np.arange(self.nbin):
            #if (ibin % 1000) == 0:
            #    print(ibin)
            xi_out[ibin] = self.interpfns[ibin]([bias_in, sigz_in])
