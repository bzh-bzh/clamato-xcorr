...
;     K.G. Lee 27/07/2015  - Extended common block to include lamb_piv
;                            variable which is used by
;                            PCASPEC_FUNC. Otherwise the latter will
;                            crash if MFREG is called first.
;-
...
if keyword_set(lyaf_range) then begin
   lyaf_min = (lyaf_range[0])[0] > wavemin/(1.+zqso)
   lyaf_max = (lyaf_range[1])[0]
endif else begin
   lyaf_min = 1041. > wavemin/(1.+zqso)
   lyaf_max = 1185.
endelse 
